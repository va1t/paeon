...
role :web, "10.10.1.132"                          # Your HTTP server, Apache/etc
role :app, "10.10.1.132"                          # This may be the same as your `Web` server
role :db,  "10.10.1.132", :primary => true # This is where Rails migrations will run

set :user, "marty"
set :deploy_to, "/var/www/html/training/#{application}"
set :use_sudo, false
...
namespace :deploy do
   ...
   task :migrate do
     ...
     run "cp -f #{deploy_to}/current/config/database/training.yml #{deploy_to}/current/config/database.yml"
