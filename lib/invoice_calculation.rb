...
module InvoiceCalculation
  ...
  def calculate_invoice    
    ...
    # if the detail records ar marked to include, then calculate the fees
    @details.each do |d|
      if d.status == InvoiceDetailType::INCLUDE
        case d.record_type
          when InvoiceDetailType::CLAIM
            self.subtotal_claims += calculate_claim_percentage(d) if self.invoice_method == PERCENT
          when InvoiceDetailType::BALANCE
            self.subtotal_balanace += calculate_balance_percentage(d) if self.invoice_method == PERCENT
          when InvoiceDetailType::COB
            self.subtotal_cob += calculate_cob_fee(d)
          when InvoiceDetailType::DENIED
            self.subtotal_denied += calculate_denied_fee(d)
          when InvoiceDetailType::SETUP
            self.subtotal_setup += calculate_patient_setup(d)
          when InvoiceDetailType::DISCOVERY
            self.subtotal_discovery += calculate_discovery_fee(d)
          when InvoiceDetailType::ADMIN
  ...
  def calculate_dos_fee
    ...
    ary = Array.new
    @details.each do |d|
      ary.push d.dos if (d.record_type == InvoiceDetailType::CLAIM || d.record_type == InvoiceDetailType::BALANCE) && d.status == InvoiceDetailType::INCLUDE 
    end
  ...
  def set_fee_dates
    ...
    if @invoice.blank?
      ...
      date = DateTime.now    
      @details.each do |d|
        date = d.dos if (d.record_type == InvoiceDetailType::CLAIM || d.record_type == InvoiceDetailType::BALANCE) && d.dos < date
      end
      self.fee_start = DateTime.new(date.year, date.month, 1)
    else
      # grab the last invoice and get last dates from that
      last_date = @invoice.last.fee_end
  ...
  def calculate_discovery_fee(detail)
    detail.charge_amount = self.discovery_fee
  end
  
  def calculate_admin_fee(detail)
    detail.charge_amount = self.admin_fee
  end
