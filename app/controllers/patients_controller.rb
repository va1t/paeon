class PatientsController < ApplicationController
  ...
  def select_primary_codes_for_display
    if params[:provider_id]
      ...
      @primary_selection = Selector::GROUP
      @group = @patient.groups.find(session[:group])
      @primary_name = @group.group_name
      @rates = @group.rates
      ...
      @primary_selection = Selector::PROVIDER
      @provider = @patient.providers.find(session[:provider])
      @primary_name = @provider.provider_name 
      @rates = @provider.rates
  ...
  def setup_static_variables
    ...
    @cpts_codes = CodesCpt.all
    @mod_codes = CodesModifier.all
    #place of service codes
    @pos_codes = CodesPos.all    
  ...
  def build_validated_subscriber_list(subscribers)      
      # builds the array of @validated which contains all the subscribers foe the patient and if the subscriber has been validated for the selected provider / or group
      # found will be true.  This allows easy displaying of prooper buttons to perform validation of subscriber.
      validated = Array.new
      subscribers.each do |s|
        ...
        validated.push :ins_priority => s.ins_priority, :ins_name => s.insurance_company.name, :subscriber_id => s.id, :found => found, :found_id => found_id, :in_network => in_network, :status => status 
