class ProcessingController < ApplicationController
  ...
  def claim_submitted
    ...
    if params[:aged] && params[:aged2]
      days = params[:aged].to_i
      days2 = params[:aged2].to_i
      @title = "Claims Submitted between #{days} to #{days2} days ago"
      @claims_submitted = InsuranceBilling.aged_between(days.days.ago, days2.days.ago)
    elsif params[:aged]
      days = params[:aged].to_i
      @title = "Claims Submitted more than #{days} days ago"
