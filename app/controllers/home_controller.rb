class HomeController < ApplicationController
  ...
  def index
    ...
    @balance = Hash.new
    @balance[:initiated]    = BalanceBill.initial.count
    @balance[:ready]        = BalanceBill.pending.count
    @balance[:invoiced]     = BalanceBill.invoiced.count
    @balance[:agedinvoice]  = BalanceBill.aged_invoice(15.days.ago).count
    @balance[:balance]      = BalanceBill.balance.count
    @balance[:paid]         = BalanceBill.paid.count
    ...
    respond_to do |format|
      format.html {
        if @insurance_session.status == SessionFlow::BALANCE
          redirect_to insurance_session_edit_balance_bills_path(@insurance_session)
        else
