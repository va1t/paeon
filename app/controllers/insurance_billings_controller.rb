class InsuranceBillingsController < ApplicationController
  ...
  def index
    ...
    # if the last claim has paid or closed status, check eobs to see if action buttons should be displayed
  ...
  def update
    ...
    respond_to do |format|
      if @insurance_billing.update_attributes(params[:insurance_billing])
        logger.info "Saved the record, ID: #{@insurance_billing.id}, ins billed: #{@insurance_billing.insurance_billed}"
        format.html {
  ...
  #
  # GET /insurance_billings/:insurance_billing_id/balance
  ...
  def balance
    ...
    respond_to do |format|
      if @claim.update_attributes(:status => BillingFlow::CLOSED, :updated_user => current_user.login_name)
        ...
        message = ""
        @claim.errors.full_messages.each {|msg| message += msg + ", "} if @claim.errors.any?
        format.html { redirect_to insurance_session_insurance_billings_path(params[:insurance_session_id]),
                      notice: message}
      end
    end
  end
  ...
  #
  # GET /insurance_billings/:insurance_billing_id/cob
  # coordination of benefits
  def cob
    logger.info "COB called; Session: #{params[:insurance_session_id]}, Ins Billing: #{params[:insurance_billing_id]}"
    @insurance_session = InsuranceSession.find(params[:insurance_session_id])
    @claim = InsuranceBilling.find(params[:insurance_billing_id])

    respond_to do |format|
      if @claim.update_attributes(:status => BillingFlow::COB, :updated_user => current_user.login_name)
        format.html { redirect_to insurance_session_insurance_billings_path(params[:insurance_session_id]),
                      notice: "Claim wa ssuccessfully set to COB status." }
      else
        message = ""
        @claim.errors.full_messages.each {|msg| message += msg + ", "} if @claim.errors.any?
        format.html { redirect_to insurance_session_insurance_billings_path(params[:insurance_session_id]), notice: message}
  ...
  #
  # GET /unwaive/:insurance_billing_id
  # Any sessions closed by waived fee, will be unwaived.
  def unwaive
    @insurance_session = InsuranceSession.find(params[:insurance_session_id])
    @insurance_billing = InsuranceBilling.find(params[:insurance_billing_id])

    # revert insurance billing state
    @insurance_billing.revert_to_previous_status
    if @insurance_billing.save!
      # revert session state
      @insurance_session.revert_to_previous_status
      # set balance_owed and wipe out waived fee
      @insurance_session.balance_owed = @insurance_session.waived_fee
      @insurance_session.waived_fee = 0.0
      @insurance_session.save!
    end
  ...
  def setup_session
    ...
    session[:return_to] = insurance_session_insurance_billings_path(@insurance_session.id)
  end
  ...
  def setup_base_variables
    ...
    if @insurance_session.selector == Selector::GROUP
      @managed_care = @insurance_session.patient.managed_cares(:conditions => ["group_id = ?",@insurance_session.group_id])
    else
      @managed_care = @insurance_session.patient.managed_cares(:conditions => ["provider_id = ?",@insurance_session.provider_id])
    end
  end
  ...
  #
  # loop through eobs, if any.  look for any eobs forwarded to secondary
  # if there is a forward to secondary then look for a paid eob after that
  # if the eobs are paid then display the action buttons.
  def check_eob_status
    @eob_forwaded = false
    @eob_paid = false
    #loop throgh all the eobs
    @insurance_session.insurance_billings.last.eobs.each do |eob|
      # if an eob was forwarded, then look for the eob marked as secondary or tertiary
      ...
      if @eob_forwarded && eob.claim_status_code.to_i >= ClaimStatus::SECONDARY && eob.claim_status_code.to_i <= ClaimStatus::TERTIARY
        @eob_paid = true
      end
    end
  end
