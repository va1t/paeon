class Eob < ActiveRecord::Base
  ...
  scope :assigned,   :joins => [:insurance_billing, :patient, :provider], 
                     :conditions => ["insurance_billings.status = ?", BillingFlow::PAID],
                     :order => "providers.last_name ASC, patients.last_name ASC, dos ASC"
  ...
  scope :unassigned, :conditions => ["insurance_billing_id IS NULL"], 
                     :order => "provider_last_name ASC, patient_last_name ASC, dos ASC"
  
  before_validation :format_date
  before_save :update_eob_fields        # update the eob fields from the claim
  before_save :update_table_links       # update all the *_id fields using the ins_bill rec, if assigned
  after_save :update_other_tables       # update status fields in ins_bill; balance due fields...
  ...
  def update_other_tables
    ...
    # if the status is already paid or closed, leave it alone.
