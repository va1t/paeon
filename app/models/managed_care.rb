...
class ManagedCare < ActiveRecord::Base
    ...
    scope :warning, :conditions => ["managed_cares.active = ? and (managed_cares.authorized_sessions - managed_cares.used_sessions <= ? || managed_cares.end_date < ?)", ACTIVE, WARNING_SESSION, Time.now + WARNING_DAYS]
