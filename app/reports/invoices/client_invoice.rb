class ClientInvoice < Prawn::Document
  ...
  def build_invoice_data
    ...
    text "Invoice #: " + str
    indent 20 do
      text "Payment Terms: " + Invoice::term(terms)
  ...
  def build_providers_address
    ...
    indent 20 do
      ...
      text ""
      text "License: " + @object.license_number
  ...
  def build_summary
    data = [["", "Description", "Qty", "Fee", "Amount"]]

    data.push ["Claims", "Insurance Billed:", "", ("$ " + sprintf("%.2f", @invoice.total_claim_charge_amount)), ""]
    data.push ["", "Insurance Paid:", "", ("$ " + sprintf("%.2f", @invoice.total_claim_payment_amount)), ""]
    case @invoice.invoice_method
      when Invoice::FLATFEE
        data.push ["", "Flat Fee Charge", @invoice.count_claims,
          ("$ " + sprintf("%.2f", @invoice.flat_fee)),
          ("$ " + sprintf("%.2f", @invoice.subtotal_claims) if !@invoice.subtotal_claims.blank?)]
      when Invoice::DOSFEE
        data.push ["", "DOS Fee Charge",  @invoice.count_claims,
          ("$ " + sprintf("%.2f", @invoice.dos_fee)),
          ("$ " + sprintf("%.2f", @invoice.subtotal_claims) if !@invoice.subtotal_claims.blank?)]
      when Invoice::PERCENT
        data.push ["", "Percentage Charge",  @invoice.count_claims,
          (sprintf("%.2f", @invoice.claim_percentage) + " %"),
          ("$ " + sprintf("%.2f", @invoice.subtotal_claims) if !@invoice.subtotal_claims.blank?)]
    end
    ...
    # balance bill
    data.push ["Balance Bill", "", @invoice.count_balances,
        (sprintf("%.2f", @invoice.balance_percentage) + " %" if !@invoice.balance_percentage.blank?),
        ("$ " + sprintf("%.2f", @invoice.subtotal_balance) if !@invoice.subtotal_balance.blank?)]

    # setup fee
    data.push ["Setup", "New Client", @invoice.count_setup,
    ...
    table data do |t|
      ...
      t.column_widths = {0 => 80, 1 => 200, 2 => 60, 3 => 60, 4 => 60}
    end
  end
