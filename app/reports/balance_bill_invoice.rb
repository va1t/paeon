class BalanceBillInvoice < Prawn::Document
  ...
  def initialize(params)
    ...
    @balance_bill = BalanceBill.includes(:patient, :balance_bill_sessions => [:insurance_session, :provider]).find(params)    
  ...
  def get_provider_name
    ...
    indent 10 do
      ...
      text "Office: " + @provider.office_phone
      text "Fax: " + @provider.fax_phone
    end  
  end
  ...
  def get_comments
    ...
    indent 10 do
      text_box @balance_bill.comment, :at => [bounds.left, bounds.top - 10], :width => bounds.width  
    end
  end
  ...
  def get_details
    ...
    @balance_bill.balance_bill_sessions.each do |session|
      dos = session.dos ? session.dos.strftime("%m/%d/%y") : " " 
      data.push [dos, "Provider: " + session.provider.provider_name, "", ""]
      session.balance_bill_details.each do |detail|
        data.push ["", detail.description, detail.quantity, "$ " + sprintf("%.2f", detail.amount) ]
      end  
    end

    #push 2 extra blank rows
    data.push [" ", "", "", ""], [" ", "", "", ""]
    table data do |t|       
  ...
  def get_detail_summary
    data = [["Curent", "30 Days Past Due", "60 Days Past Due", "90 Days Past Due", "More than 90 Days Past Due", "Amount Due"]]
    data.push ["$ " + sprintf("%.2f", @balance_bill.total_amount),
               "", 
               "", 
               "", 
               "", 
               "$ " + sprintf("%.2f", @balance_bill.total_amount)]
    table data do |t|
      t.row(0).style :background_color => "CCCCCC", :align => :center, :font_style => :bold
  ...
  def get_remittance
    ...
    data = [["Remittance", " "],
            ...
            ["Date", " "],
            ["Amount Due:", "$ " + sprintf("%.2f", @balance_bill.total_amount)],
            ["Amount Enclosed", " "]]
    table data do |t|
      t.row(0).style :background_color => "CCCCCC", :align => :center, :font_style => :bold
